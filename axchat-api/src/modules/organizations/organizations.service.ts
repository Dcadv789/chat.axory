...
export class OrganizationsService {
  ...
  async updateOrganization(orgId: string, dto: UpdateOrganizationDto) {
    ...
    const updated = await this.repository.update(orgId, {
      ...
      ...(allowedUrlDomains !== undefined
        ...
        : {}),
      // String vazia ou null => limpa (volta pro fallback de env).
  ...
  /**
   * Nunca expoe a chave DeepSeek em texto puro pela API. Devolve um preview
   * mascarado + um booleano pra UI saber se ha chave configurada.
   */
  private maskSecrets<T extends { deepseekApiKey?: string | null }>(org: T) {
    const key = org.deepseekApiKey ?? null;
    return {
      ...org,
      deepseekApiKey: null,
      deepseekApiKeySet: !!key,
      deepseekApiKeyPreview: key
        ? `${'•'.repeat(8)}${key.slice(-4)}`
        : null,
    };
  }
