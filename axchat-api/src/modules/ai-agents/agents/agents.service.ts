...
export class AgentsService {
  ...
  async getBusinessMetrics(
    ...
    const [
      ...
      this.prisma.organization.findUnique({
        where: { id: organizationId },
        select: { monthlyConversationLimit: true },
      }),
    ]);

    const totalRuns = runs.length;
    const aiHandledConversations = runs.filter(
      (run) => run.finalAction && run.finalAction !== AiFinalAction.NO_ACTION,
    ).length;
    const resolvedWithoutHuman = runs.filter(
      (run) =>
        run.finalAction === AiFinalAction.REPLIED ||
        run.finalAction === AiFinalAction.CLOSED_CONVERSATION,
    ).length;
    const transferredToHuman = runs.filter(
      (run) => run.finalAction === AiFinalAction.TRANSFERRED_TO_HUMAN,
    ).length;

    const responseTimesWithAi: number[] = [];
    const responseTimesWithoutAi: number[] = [];
    for (const conversation of conversations) {
      if (!conversation.firstResponseAt) continue;
      const minutes =
        (conversation.firstResponseAt.getTime() - conversation.createdAt.getTime()) /
        60000;
      if (minutes < 0) continue;
      if (conversation.aiRuns.length > 0) responseTimesWithAi.push(minutes);
      else responseTimesWithoutAi.push(minutes);
    }

    const byChannel = groupCount(
      conversationsByChannel,
      (conversation) => conversation.channel.type,
    );
    ...
    return {
      ...
      resolutionWithoutHumanRate:
        totalRuns > 0 ? +((resolvedWithoutHuman / totalRuns) * 100).toFixed(1) : null,
      transferToHumanRate:
        totalRuns > 0 ? +((transferredToHuman / totalRuns) * 100).toFixed(1) : null,
      averageFirstResponseMinutes: {
        withAi: average(responseTimesWithAi),
        withoutAi: average(responseTimesWithoutAi),
      },
      satisfaction: {
        averageScore:
          ratings._avg.score !== null ? +ratings._avg.score.toFixed(1) : null,
        responses: ratings._count._all,
      },
      conversationsByChannel: Object.entries(byChannel).map(([channelType, count]) => ({
        channelType,
        count,
      })),
      manualAttentionConversations: stuckConversations,
      planUsage: {
        used: monthlyAiHandled,
        limit: monthlyLimit,
        percentUsed:
          monthlyLimit && monthlyLimit > 0
            ? +((monthlyAiHandled / monthlyLimit) * 100).toFixed(1)
            : null,
      },
    };
  }
