...
export class InstagramHttpClient {
  ...
  // ─── Business Login for Instagram (OAuth próprio, sem Página) ─────
  // Fluxo alternativo ao Facebook Login: conecta a conta do Instagram
  // DIRETO (popup do próprio Instagram), sem depender de Página nem de
  // Business Portfolio. Resolve o caso de conta IG num portfólio empresarial
  // separado da Página. Usa graph.instagram.com + token IGAA (modo 'instagram').

  /** Escopos do Business Login for Instagram (nomes novos, pós jan/2025). */
  static readonly IG_LOGIN_SCOPES = [
  ...
  /** URL da janela de autorização do Instagram (Business Login). */
  buildInstagramLoginUrl(
    igAppId: string,
    redirectUri: string,
    state: string,
    scope: string = InstagramHttpClient.IG_LOGIN_SCOPES,
  ): string {
  ...
  /** Troca o code por token de CURTA duração (1h) + user_id (IG-scoped). */
  async exchangeInstagramLoginCode(
    code: string,
    redirectUri: string,
    igAppId: string,
    igAppSecret: string,
    ...
    try {
      const form = new URLSearchParams({
        ...
        redirect_uri: redirectUri,
        // A Meta rejeita o code com '#_' no fim que ela mesma às vezes anexa.
        code: code.replace(/#_$/, ''),
      });
      const { data } = await axios.post(
      ...
      return { accessToken: String(token), userId: String(userId) };
    } catch (err: any) {
      const msg = err?.response?.data?.error_message || err?.response?.data?.error?.message || err.message;
      this.logger.error(`Instagram Login code exchange failed: ${msg}`);
      throw new Error(`Falha ao trocar código do Instagram: ${msg}`);
    }
  }

  /** Troca o token curto por um de LONGA duração (60 dias). */
  async exchangeInstagramLoginLongLived(
    shortToken: string,
    igAppSecret: string,
    ...
    try {
      ...
      if (!data?.access_token) throw new Error('Instagram não retornou o token longo');
      return {
      ...
      const msg = err?.response?.data?.error?.message || err.message;
      this.logger.error(`Instagram Login long-lived exchange failed: ${msg}`);
      throw new Error(`Falha ao obter token longo do Instagram: ${msg}`);
    }
  }

  /** Perfil da conta IG conectada via Instagram Login (user_id + username). */
  async getInstagramLoginMe(
    token: string,
    ...
    try {
      ...
      const msg = err?.response?.data?.error?.message || err.message;
      throw new Error(`Falha ao ler o perfil do Instagram: ${msg}`);
    }
  }
