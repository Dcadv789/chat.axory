...
export const DEPARTMENT_COLORS: Record<string, { bg: string; text: string; ring: string }> = {
  ...
  OUTRO:       { bg: 'bg-zinc-100 dark:bg-black',      text: 'text-zinc-600 dark:text-zinc-400',       ring: 'ring-zinc-200 dark:ring-zinc-700' },
};
...
export interface BusinessMetrics {
  ...
  resolutionWithoutHumanRate: number | null;
  transferToHumanRate: number | null;
  ...
  conversationsByChannel: Array<{ channelType: string; count: number }>;
  manualAttentionConversations: number;
  planUsage: {
    used: number;
    limit: number | null;
    percentUsed: number | null;
