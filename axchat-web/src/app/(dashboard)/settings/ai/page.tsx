...
export default function SettingsAiPage() {
  ...
  useEffect(() => {
    ...
    setDeepseekKeyPreview(data.deepseekApiKeyPreview ?? null);
  ...
  const handleRemoveDeepseekKey = async () => {
    setSaving(true);
    try {
      await aiSettingsService.update({ deepseekApiKey: null });
      toast.success('Chave DeepSeek removida');
      setDeepseekKey('');
      qc.invalidateQueries({ queryKey: ['ai-settings'] });
    } catch (err: any) {
      ...
      <section className="mt-4 rounded-xl border border-zinc-200 bg-white p-5 dark:border-white/10 dark:bg-black">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="mt-0.5 text-xs text-zinc-500">
              Provider principal de texto dos agentes. A chave é salva por
              organização e nunca é exibida de volta.{' '}
              {deepseekKeySet ? (
                <span className="font-medium text-emerald-600 dark:text-emerald-400">
                  Configurada ({deepseekKeyPreview}). Digite uma nova para
                  substituir.
                </span>
              ) : (
                <span className="font-medium text-amber-600 dark:text-amber-400">
                  Nenhuma chave configurada — os agentes não respondem até você
                  adicionar uma.
                </span>
              )}
            </p>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {deepseekKeySet ? (
                <button
                  ...
                  disabled={saving}
                  className="rounded-md border border-red-200 px-3 py-2 text-sm font-medium text-red-600 hover:bg-red-50 disabled:opacity-50 dark:border-red-900/50 dark:hover:bg-red-950/30"
                >
                  Remover
                </button>
              ) : null}
            ...
            <p className="mt-1 text-[10px] text-zinc-400">
              Pegue em platform.deepseek.com → API Keys. A chave é usada para o
              modelo <code className="font-mono">deepseek-chat</code>. Clique em
              “Salvar alterações” no topo para aplicar.
            </p>
          </div>
        </div>
      </section>
